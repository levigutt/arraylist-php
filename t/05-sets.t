...
$assert->is(2, $inter->count(), "( [1,2,3,4] (|) [3,4,5,6] )->count() == 2");
$assert->is("43", $inter->pop().$inter->pop(),
                                  "( [1,2,3,4] (|) [3,4,5,6] ) == [3,4]");

$inter = $list->intersect(  new ArrayList(...array_slice($numbers, 0, 4))
                         ,  new ArrayList(...array_slice($numbers, 2, 4)));
$assert->is(2, $inter->count(),     "( [1,2,3,4] (|) [3,4,5,6] )->count() == 2");
$assert->is("34", $inter->join(''), "( [1,2,3,4] (|) [3,4,5,6] ) == [3,4]");

# union
$list = new ArrayList(1,2,3);
$union = $list->union([3,4,5]);
$assert->is(5, $union->count(),    "union removed duplicate in intersection");
...
$list->merge($list2);
$assert->is($list2->pop(), $list->pop(),
    "list merged with list has new end item");
$assert->is($numbers[0], $list->shift(),
    "list merged wih list has same start item");

$list = l(...$numbers);
$list2 = [100, 200, 300];
$list->merge($list2);
$assert->is(300, $list->max(),
    "list merged with array has new max");
$assert->is($list2[count($list2)-1], $list->pop(),
...
$assert->is(false, $list->subset([0,5]),   "[0,5] is not a subset of 1..10");
$assert->is(false, $list->subset([2,11]),  "[2,11] is not a subset of 1..10");
...
$assert->is(false, $list->superset(range(3,5)), "[3..5] is not superset of 2..5");
$assert->is(false, $list->superset(l(1,3,4)),   "(1,3,4) is not superset of 2..5");
...
$assert->is(false, $list->same(l(0,1,2)),   "1..3 not same as (0,1,2)");
$assert->is(false, $list->same(l(1,2,3,4)), "1..3 not same as (1,2,3,4)");
$assert->is(false, $list->same(l(3)),       "1..3 not same as (3)");
$assert->is(false, $list->same(l(6)),       "1..3 not same as (6)");

# identical
$list = l(1,2,3);
$assert->ok($list->identical([1,2,3]),           "1..3 identical to [1,2,3]");
$assert->ok($list->identical(l(1,2,3)),          "1..3 identical to (1,2,3)");
$assert->is(false, $list->identical(l(3,2,1)),   "1..3 not identical to (3,2,1)");
$assert->is(false, $list->identical([1,2,3,2,1]),"1..3 not identical to [1,2,3,2,1]");
$assert->is(false, $list->identical(l(3,2,1)),   "1..3 not identical to (3,2,1)");

# diff
$list = new ArrayList(...$numbers);
$diff = $list->diff($numbers);
$assert->is(0, $diff->count(), "diff on identical returns empty");

$diff = $list->diff(array_slice($numbers, 0, 6), array_slice($numbers, 5, 4));
$assert->is(1, $diff->count(), "diff with one difference returns one");
$assert->is(10, $diff->pop(),  "diff with one difference returns missing item");

$diff = $list->diff(new ArrayList(...array_slice($numbers, 0, 6)), new ArrayList(...array_slice($numbers, 5, 4)));
$assert->is(1, $diff->count(), "diff: same count when passing ArrayList");
$assert->is(10, $diff->pop(),  "diff: same pop when passing ArrayList");

$list1 = l(2,2,3);
$list2 = l(1,1,2);
$diff = $list1->diff($list2);
$assert->is(1, $diff->count(), "diff removes duplicates");

# symmetrical diff
$list1 = l(1,2,3);
$list2 = l(2,3,4);
$diff = $list1->symdiff($list2);
$assert->is(2, $diff->count(),
    "(1,2,3) symdiff (2,3,4) == count is 2");
$assert->is("14", $diff->reduce(fn($n,$a) => $a.$n),
    "(1,2,3) symdiff (2,3,4) == 1,4");

$list3 = l(3,1,2);
$diff = $list1->symdiff($list3);
$assert->is(0, $diff->count(), "symdiff on same list has 0 results"); 
